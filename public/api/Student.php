...
class Student extends BaseController
{
    ...
    public function login()
    {
        try {
            ...
            // Remove any binary/blob fields that would break JSON encoding
            if (isset($student['profile_picture'])) unset($student['profile_picture']);
            if (isset($student['qr_code'])) unset($student['qr_code']);
    ...
    public function profile($studentId)
    {
        try {
            ...
            // Do not expose the password hash. Provide an empty password field
            // so the front-end can render a password input if needed.
    ...
    public function getQrCode($studentId)
    {
        try {
            ...
            // Set appropriate headers for PNG image and return binary data
            ...
            return $this->response->setJSON([
                'status' => 'error',
                'message' => 'An error occurred while fetching QR code: ' . $e->getMessage()
            ])->setStatusCode(ResponseInterface::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public function updateProfile()
    {
        try {
            ...
            if ($profilePicture) {
                // Decode base64 image
                $imageData = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $profilePicture));
                if ($imageData !== false) {
            ...
            // Strip binary data before returning
            if (isset($updatedStudent['profile_picture'])) unset($updatedStudent['profile_picture']);
            if (isset($updatedStudent['qr_code'])) unset($updatedStudent['qr_code']);
            // Do not expose password hash
            if (isset($updatedStudent['password'])) {
